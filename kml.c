...
static VOID __cdecl PrintfToLog(LPCTSTR lpFormat, ...)
{
	...
	lpOutput = HeapAlloc(hHeap, 0, 1024);
...
static BOOL DisplayKMLLog(BOOL bOkEnabled)
{
	BOOL bResult;
...
static VOID CreateKmlList()
{
	...
	do
	{
		...
		if (lstrcmpi(_T(HARDWARE),GetStringParam(pBlock,TOK_GLOBAL,TOK_HARDWARE,0)))
		{
			FreeBlocks(pBlock);
			continue;
		}
...
static BOOL CALLBACK ChooseKMLProc(HWND hDlg, UINT message, DWORD wParam, LONG lParam)
{
	...
	{
	case WM_INITDIALOG:
		SetDlgItemText(hDlg,IDC_HP8XDIR,szHP8xDirectory);
	...
	case WM_COMMAND:
		...
		{
			DestroyKmlList();
			GetDlgItemText(hDlg,IDC_HP8XDIR,szHP8xDirectory,sizeof(szHP8xDirectory));
			CreateKmlList();
			hList = GetDlgItem(hDlg,IDC_KMLSCRIPT);
			SendMessage(hList, CB_RESETCONTENT, 0, 0);
			pList = pKmlList;
...
static LPTSTR ParseString(VOID)
{
	...
	nLength = 0;
	nBlock = 255;
	szBuffer = HeapAlloc(hHeap, 0, nBlock+1);
	while (*szText != _T('"'))
	{
		if (*szText == _T('\\')) szText++;
		if (*szText == 0)
		...
		{
			nBlock += 256;
			szBuffer = HeapReAlloc(hHeap, 0, szBuffer, nBlock+1);
		}
		szText++;
	}
	szText++;
...
static KmlLine* ParseLine(TokenId eCommand)
{
	...
	j = 0;
	pLine = HeapAlloc(hHeap, 0, sizeof(KmlLine));
...
static KmlBlock* ParseBlock(TokenId eType)
{
	...
	pBlock = HeapAlloc(hHeap, 0, sizeof(KmlBlock));
	pBlock->eType = eType;
...
static VOID InitGlobal(KmlBlock* pBlock)
{
	...
	while (pLine)
	{
		...
		case TOK_MODEL:
			...
			PrintfToLog(_T("Calculator Model : %u"), nCurrentRomType);
			break;
		case TOK_CLASS:
...
static KmlLine* InitLcd(KmlBlock* pBlock)
{
	...
	while (pLine)
	{
		...
		case TOK_OFFSET:
			...
			break;
		case TOK_ZOOM: /*	no more used
			nLcdxZoom = pLine->nParam[0];
			switch (nLcdxZoom)
			{
				case 1 : nLcdyZoom = 1; break;
				case 2 : nLcdyZoom = 2; break;
				case 3 : nLcdyZoom = 3; break;
				default: nLcdxZoom = nLcdyZoom = 1;
			} */
			break;
		case TOK_COLOR:
...
//	EnterCriticalSection(&csGDILock);		// solving NT GDI problems
	{
		for (i = 0; i< 6; i++) {
			...
			BitBlt(hWindowDC, x, y, 8, 12, hMainDC, 8 * c, nBackgroundY + nBackgroundH, SRCCOPY);
			x += 8;
		}
		GdiFlush();
	}
...
//	{
		for (i = 0; i< 5; i++) {
			...
			BitBlt(hWindowDC, x, y, 8, 11, hMainDC, 8 * c, nBackgroundY + nBackgroundH, SRCCOPY);
			x += 8;
		}
		GdiFlush();
//	}
...
/* VOID RunKey(BYTE nId, BOOL bPressed)
{
	#if defined DEBUG_KB
	{
		...
		i = wsprintf(buffer,_T("KEYB: keycode : %02X,%u\n"), nId, bPressed);
		buffer[i++] = 0;
